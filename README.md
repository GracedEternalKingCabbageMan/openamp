...
- Working today (committed code): registration, enclave addresses and balances, hosted transfers with fee conversion or sponsorship, raw co-signing of self-built transactions, hosted issuance (demo mode), freezes, categories, per-asset rules (velocity, holder cap, lock-in, vesting), clawback, ownership reports, transparency log with on-chain anchoring, and a reorg-aware chain follower.
- In progress (not in this repository yet): a FROST threshold backend for the policy key, and opt-in confidential (blinded) restricted assets end to end. The committed daemon signs with a single local policy key per asset and issues transparent assets only; the policy engine currently refuses blinded outputs in restricted transfers. See "Trust model" below for what is design intent versus committed code.
...
- **Confidentiality is opt-in by design, transparent today.** Sequentia is transparent by default with opt-in confidentiality, and OpenAMP follows: the contract format reserves a `confidential` flag (the policy server would hold the blinding keys, so the issuer sees every holding while outside observers see nothing). The committed daemon issues with `confidential: false` and refuses blinded outputs in restricted transfers; confidential assets end to end are in-progress work.
