...
/// its CMR.
pub const BUDGET_PAD_WORDS: usize = 704;
...
pub fn verifier_witness(
    ...
    let out_ty = "Option<(Pubkey, u32, u32, [(u256, bool); 16])>";
    let in_ty = "Option<(Pubkey, u32, u32, [(u256, bool); 16], u256, u256, [(u256, bool); 16])>";
    ...
    for (i, slot) in in_slots.iter().enumerate() {
        let value = match slot {
            None => "None".to_string(),
            Some((_key, owner_proof, interval)) => format!(
                "Some(({}, 0x{}, 0x{}, {}))",
                wl_slot_literal(owner_proof),
